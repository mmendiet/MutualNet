...
class EarlyBird():
    ...
    def early_bird_emerge(self, model, logger, mode=0):
        found = {}
        for percent in sorted(self.percent_list):
        ...
        if 'mobilenet' in FLAGS.model:
            ...
            for k, nm in enumerate(model.named_modules()):
                if isinstance(nm[1], nn.BatchNorm2d) and 'resobn.0' in nm[0]:
                    ...
                    pruned = pruned + mask.shape[0] - torch.sum(mask)
                    num = int(torch.sum(mask))
                    if keyword in n and bns in n:
                        cfg.append(max(num,1))
                    elif 'downsample' not in n:
                        cfg.append(mask.shape[0])
        return cfg